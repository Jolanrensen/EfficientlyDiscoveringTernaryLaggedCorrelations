...
import nl.jolanrensen.efficientlyDiscoveringTernaryLaggedCorrelations.dataset.StockData
import nl.jolanrensen.efficientlyDiscoveringTernaryLaggedCorrelations.dataset.StockDataWithoutTime
import nl.jolanrensen.efficientlyDiscoveringTernaryLaggedCorrelations.isNotNaN
import nl.jolanrensen.efficientlyDiscoveringTernaryLaggedCorrelations.stampPearson.StampArrayCache
 ...
 */
object ExampleSecondDataset : Serializable {
    ...
    fun main(args: Array<String>) = withSpark(
        ...
        val stocksFileLocation = "./data/dataset2.parquet"

        val stocksList: List<StockExtraInfo> = StockData.read(
            stocksFileLocation = stocksFileLocation,
            spark = spark,
            randomize = false,
            limit = 5,
        )
            .mapInPartitions { it: StockData ->
                // This dataset has some NaN values at the ends which need to be removed first
                val nonNaNSection: IntRange = it.closingPrices.indexOfFirst { it.isNotNaN() }..it.closingPrices.indexOfLast { it.isNotNaN() }
                val closingPrices: DoubleArray = it.closingPrices.slice(nonNaNSection).toDoubleArray()
                val instantLongs: LongArray = it.instantLongs.slice(nonNaNSection).toLongArray()
        ...
        val results: BoundedPriorityQueue<Result> = groups.mapPartitions { it: Iterator<IntArray> ->
            ...
            val stampPearson3ts = StampPearson3ts(maxArraySize = 9000, StampArrayCache.NoSlots) // (dataset2 has series of size around 8000+)
            ...
            it.forEach { group: IntArray ->
                ...
                try {
                    ...
                    println("A combination was skipped due to erroneous correlation values, $e")
                }
            }
        ...
        for ((res: List<StockExtraInfo>, resIndex: IndexAgg, corr: Double) in results) {
            ...
            println("""
                ...
                listOf(stock1.instantLongs[aIndex], stock1.instantLongs[aIndex + windowSize - 1]).map(::Timestamp)
            } ${stock1.closingPrices.slice(aIndex until aIndex + windowSize)}
                |b: ${stock2.stockExchange}, ${stock2.stockName}: ${
                listOf(stock2.instantLongs[bIndex], stock2.instantLongs[bIndex + windowSize - 1]).map(::Timestamp)
            } ${stock2.closingPrices.slice(bIndex until bIndex + windowSize)}
                |c: ${stock3.stockExchange}, ${stock3.stockName}: ${
                listOf(stock3.instantLongs[cIndex], stock3.instantLongs[cIndex + windowSize - 1]).map(::Timestamp)
